			...
			ExecuteOnMainThread(() => {
				if (GameSparksSettings.DebugBuild)
				{
					if (message.Length < 1500)
					{
                        ...
                        //Poor separation, but if we get the message "Available" then we have successfully connect and can log into GameSparks
                        if (message == "Available") 
                        {
                            ASL.LobbyManager.m_ConnectButton.GetComponent<UnityEngine.UI.Button>().interactable = true;
                        }
                    } else
					{
