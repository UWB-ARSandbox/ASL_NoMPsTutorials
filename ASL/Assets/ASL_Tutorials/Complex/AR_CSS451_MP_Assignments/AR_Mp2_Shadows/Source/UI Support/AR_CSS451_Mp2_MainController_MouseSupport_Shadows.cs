...
namespace Mps
{
    public partial class AR_CSS451_Mp2_MainController_Shadows : MonoBehaviour
    {
        ...
        void FingerSelect()
        {
            ...
            if (!EventSystem.current.IsPointerOverGameObject(touch.fingerId)) // check for GUI
            {
                ...
                else //if we did not hit a game object
                {
                    ...
                    if (ASL.ASLHelper.m_CloudAnchors.Count <= 0 && ASL.GameLiftManager.GetInstance().AmLowestPeer()) 
                    {
        ...
        public void SetInitialPositionToCloudAnchorsInitialPosition(GameObject _gameObject, TrackableHit _cloudAnchorSpawnLocation)
        {
            ...
            mModel.GetComponent<ASL.ASLObject>().SendAndSetClaim(() => 
            { 
                mModel.GetComponent<ASL.ASLObject>().SendAndSetWorldPosition(_gameObject.transform.position);
                mModel.GetComponent<ASL.ASLObject>().SendAndSetWorldRotation(_gameObject.transform.rotation);
            });
        }
