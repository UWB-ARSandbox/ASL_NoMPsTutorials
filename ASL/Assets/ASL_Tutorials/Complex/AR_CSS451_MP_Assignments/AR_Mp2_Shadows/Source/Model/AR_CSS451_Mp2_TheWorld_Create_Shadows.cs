...
namespace Mps
{
    public partial class AR_CSS451_Mp2_TheWorld_Shadows : MonoBehaviour
    {
        ...
        public void CreatePrimitive(PrimitiveType type)
        {
            if (mSelected != null)
            {
                ASL.ASLHelper.InstanitateASLObject(type, mSelected.transform.localPosition + kDeltaVector, Quaternion.identity, mSelected.GetComponent<ASL.ASLObject>().m_Id, 
                    "Mps.AR_CSS451_Mp2_ColorSupport",
            ...
            {               
                ...
                ASL.ASLHelper.InstanitateASLObject(type, new Vector3(x, y, z), Quaternion.identity, "TheWorld", "Mps.AR_CSS451_Mp2_ColorSupport",
                   ChangeColorUponCreation, null, MiscBehaviorHandler);
            }
        }
        ...
        public static void ChangeColorUponCreation(GameObject _myGameObject)
        {
            if (_myGameObject.transform.parent.name == "TheWorld")
            {
                _myGameObject.GetComponent<ASL.ASLObject>().SendAndSetClaim(() =>
                {
                    ...
                    _myGameObject.GetComponent<ASL.ASLObject>().SendAndSetLocalScale(new Vector3(1, 1, 1));
                });
            }
            else
            {
        ...
        public static void MiscBehaviorHandler(string _id, float [] _key)
        {
            ...
            if (ASL.ASLHelper.m_ASLObjects.TryGetValue(_id, out ASL.ASLObject myObject))
            {
                ...
                {
                    case 1: //Set to black
                        myObject.GetComponent<AR_CSS451_Mp2_ColorSupport>().m_OrgObjColor = Color.black; //Set original color for future use
                        break;
                    case 2: //Set to color passed in
                        myObject.GetComponent<AR_CSS451_Mp2_ColorSupport>().m_OrgObjColor = new Color(_key[1], _key[2], _key[3]); //Set original color for future use
                        break;
                    default:
