    ...
    public class CSS385_Mp2_GlobalBehavior : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (ASL.GameLiftManager.GetInstance().AmLowestPeer())
            {
                ...
                for (int i = 0; i < ASL.GameLiftManager.GetInstance().m_Players.Count; i++)
                {
        ...
        public static void ModifyPlayer(string _id, float[] _floatCodes)
        {
            ...
            if (ASL.ASLHelper.m_ASLObjects.TryGetValue(_id, out ASL.ASLObject player))
            {
                ...
                if (Regex.IsMatch(player.gameObject.name, ASL.GameLiftManager.GetInstance().m_PeerId.ToString()))
                {
        ...
        public void HeroHitByEnemy() { if (mHero != null) mHero.HeroHit(); }
